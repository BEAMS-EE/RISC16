...
public abstract class Memoire extends JInternalFrame {
	...
	public void warning(String text, int a){
		...
		text2 = "\nLine "
			+ a
			+ " :\n"+text+"\n"
			//+"La donnée est remplacée par zéro\n\n"
			;
	...
	public void dessine(Graphics g) {
		...
		if (!ROM){//ram
			if(WE){ 
				printText(g,13,"WE!",X()+5,Y()+getHt()-20,Color.red,1);
				g.drawString("[ " + data+" ]", x + 5, y + 20);
			}
			if (addressValue!=-1 && isActive && state != 0) g.drawString("[ "+addressValue+" ]",x+45,y+20);
		}
	}
