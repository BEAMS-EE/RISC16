...
\begin{itemize}
  ...
  \item[\notdone] The compiler can't handle a \texttt{movi} with a label not yet declared.
  It throws an error in \texttt{movi()} on line \texttt{MemProg.java:442}.

  \item[\notdone] When using a label as the signed immediate operand of \texttt{addi}, the RiSC stores the relative address of the label, not its absolute.
